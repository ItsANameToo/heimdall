    ...
    async register (container, options) {
        ...
        emitter.on('block.applied', async block => {
            try {
                ...
                const currentSlot = Crypto.Slots.getSlotNumber(lastBlock.timestamp)

                let activeDelegates = await dbService.getActiveDelegates(roundCalculator.calculateRound(lastBlock.height))
                ...
                const secondUntilRoundEnd = (delegatesCount - (lastBlock.height % delegatesCount)) * 8
